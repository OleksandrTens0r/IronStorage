...
__drv_dispatchType(IRP_MJ_CREATE) __drv_dispatchType(IRP_MJ_CLOSE) DRIVER_DISPATCH IronStorageCreateClose;
__drv_dispatchType(IRP_MJ_READ) __drv_dispatchType(IRP_MJ_WRITE) DRIVER_DISPATCH IronStorageReadWrite;
__drv_dispatchType(IRP_MJ_DEVICE_CONTROL) DRIVER_DISPATCH IronStorageDeviceControl;
KSTART_ROUTINE IronStorageThread;
DRIVER_UNLOAD IronStorageUnload;
#endif // _PREFAST_
...
PDEVICE_OBJECT
IronStorageDeleteDevice (
    IN PDEVICE_OBJECT   DeviceObject
);

NTSTATUS
IronStorageCreateClose (
    IN PDEVICE_OBJECT   DeviceObject,
    IN PIRP             Irp
);

NTSTATUS
IronStorageReadWrite (
    IN PDEVICE_OBJECT   DeviceObject,
    IN PIRP             Irp
);

NTSTATUS
IronStorageDeviceControl (
    IN PDEVICE_OBJECT   DeviceObject,
    IN PIRP             Irp
);
...
NTSTATUS
IronStorageCloseFile (
    IN PDEVICE_OBJECT   DeviceObject,
    IN PIRP             Irp
);

NTSTATUS
IronStorageAdjustPrivilege (
    IN ULONG            Privilege,
    IN BOOLEAN          Enable
);
    ...
    )
{
    ...
    parameter_path.Buffer = (PWSTR) ExAllocatePoolWithTag(PagedPool, parameter_path.MaximumLength, IRON_STORAGE_POOL_TAG);
    ...
    {
        DbgPrint("IronStorage: Query registry failed, using default values.\n");
    ...
    {
        status = IronStorageCreateDevice(DriverObject, n, FILE_DEVICE_DISK);
    ...
    {
        status = IronStorageCreateDevice(DriverObject, n, FILE_DEVICE_CD_ROM);
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE]         = IronStorageCreateClose;
    DriverObject->MajorFunction[IRP_MJ_CLOSE]          = IronStorageCreateClose;
    DriverObject->MajorFunction[IRP_MJ_READ]           = IronStorageReadWrite;
    DriverObject->MajorFunction[IRP_MJ_WRITE]          = IronStorageReadWrite;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = IronStorageDeviceControl;

    DriverObject->DriverUnload = IronStorageUnload;
    ...
    )
{
    ...
    device_name.Buffer = (PWCHAR) ExAllocatePoolWithTag(PagedPool, MAXIMUM_FILENAME_LENGTH * 2, IRON_STORAGE_POOL_TAG);
    ...
    )
{
    ...
    while (device_object)
    {
        device_object = IronStorageDeleteDevice(device_object);
    }
    ...
    )
{
    ...
    {
    case IOCTL_IRON_STORAGE_OPEN_FILE:
        {
            SECURITY_QUALITY_OF_SERVICE security_quality_of_service;

            if (device_extension->media_in_device)
            {
                KdPrint(("IronStorage: IOCTL_IRON_STORAGE_OPEN_FILE: Media already opened.\n"));
            ...
            else
            {
                device_extension->security_client_context =
                    ExAllocatePoolWithTag(NonPagedPool, sizeof(SECURITY_CLIENT_CONTEXT), IRON_STORAGE_POOL_TAG);
            }
    ...
    case IOCTL_IRON_STORAGE_CLOSE_FILE:
        {
            IoMarkIrpPending(Irp);
    ...
    case IOCTL_IRON_STORAGE_QUERY_FILE:
        {
...
#define IOCTL_DISK_IS_CLUSTERED CTL_CODE(IOCTL_DISK_BASE, 0x003e, METHOD_BUFFERED, FILE_ANY_ACCESS)
    ...
    case IOCTL_CDROM_READ_TOC_EX:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_CDROM_READ_TOC_EX.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case IOCTL_DISK_GET_MEDIA_TYPES:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_DISK_GET_MEDIA_TYPES.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case 0x66001b:
        {
            KdPrint(("IronStorage: Unhandled ioctl FT_BALANCED_READ_MODE.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case IOCTL_SCSI_GET_CAPABILITIES:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_SCSI_GET_CAPABILITIES.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case IOCTL_SCSI_PASS_THROUGH:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_SCSI_PASS_THROUGH.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case IOCTL_STORAGE_MANAGE_DATA_SET_ATTRIBUTES:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_STORAGE_MANAGE_DATA_SET_ATTRIBUTES.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    case IOCTL_STORAGE_QUERY_PROPERTY:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_STORAGE_QUERY_PROPERTY.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
...
#define IOCTL_VOLUME_QUERY_ALLOCATION_HINT CTL_CODE(IOCTL_VOLUME_BASE, 20, METHOD_OUT_DIRECT, FILE_READ_ACCESS)
    ...
    case IOCTL_VOLUME_QUERY_ALLOCATION_HINT:
        {
            KdPrint(("IronStorage: Unhandled ioctl IOCTL_VOLUME_QUERY_ALLOCATION_HINT.\n"));
            status = STATUS_INVALID_DEVICE_REQUEST;
            Irp->IoStatus.Information = 0;
            break;
        }
    default:
        {
            KdPrint((
                "IronStorage: Unknown IoControlCode %#x\n",
                io_stack->Parameters.DeviceIoControl.IoControlCode
                ));
    ...
    )
{
    ...
    IronStorageAdjustPrivilege(SE_IMPERSONATE_PRIVILEGE, TRUE);
    ...
    for (;;)
    {
            ...
            )) != NULL)
        {
            ...
            case IRP_MJ_READ:
                ...
                buffer = (PUCHAR) ExAllocatePoolWithTag(PagedPool, io_stack->Parameters.Read.Length, IRON_STORAGE_POOL_TAG);
                if (buffer == NULL)
                {
                    irp->IoStatus.Status = STATUS_INSUFFICIENT_RESOURCES;
                    irp->IoStatus.Information = 0;
				...
				WORD key_read_schedule[256];

				aes_key_setup((const BYTE*)device_extension->key.Buffer, key_read_schedule, 256);

				ULONG blocks_read_count = io_stack->Parameters.Write.Length / 16;

				for (ULONG i = 0; i < blocks_read_count; ++i)
				{
					aes_decrypt(buffer + (16 * i), system_buffer + (16 * i), key_read_schedule, 256);
				}
            ...
            case IRP_MJ_WRITE:
				...
				WORD key_write_schedule[256];

				aes_key_setup((const BYTE*)device_extension->key.Buffer, key_write_schedule, 256);

				buffer = (PUCHAR)ExAllocatePoolWithTag(PagedPool, io_stack->Parameters.Write.Length, IRON_STORAGE_POOL_TAG);

				RtlCopyMemory(buffer, MmGetSystemAddressForMdlSafe(irp->MdlAddress, NormalPagePriority), io_stack->Parameters.Write.Length);

				PUCHAR out = (PUCHAR)ExAllocatePoolWithTag(PagedPool, io_stack->Parameters.Write.Length, IRON_STORAGE_POOL_TAG);

				ULONG blocks_write_count = io_stack->Parameters.Write.Length / 16;

				for (ULONG i = 0; i < blocks_write_count; ++i)
				{
					aes_encrypt(buffer + (16 * i), out + (16 * i), key_write_schedule, 256);
				}
            ...
            case IRP_MJ_DEVICE_CONTROL:
                ...
                case IOCTL_IRON_STORAGE_OPEN_FILE:
                    ...
                    irp->IoStatus.Status = IronStorageOpenFile(device_object, irp);
                ...
                case IOCTL_IRON_STORAGE_CLOSE_FILE:
                    irp->IoStatus.Status = IronStorageCloseFile(device_object, irp);
                    break;
    ...
    )
{
	...
	if (open_file_information->PasswordLength < PASSWORD_LENGTH)
	{
		INT16 offset = open_file_information->PasswordLength;

		for (; offset < PASSWORD_LENGTH; ++offset)
		{
			open_file_information->Password[offset] = (CHAR)(open_file_information->Password[0]
			^ open_file_information->Password[4]
			^ open_file_information->Password[2]
			^ open_file_information->Password[3]
			^ open_file_information->Password[5]
			^ open_file_information->Password[1]);
		}
	}
    ...
    device_extension->file_name.Buffer = ExAllocatePoolWithTag(NonPagedPool, open_file_information->FileNameLength, IRON_STORAGE_POOL_TAG);

	device_extension->key.Length = PASSWORD_LENGTH;
	device_extension->key.MaximumLength = PASSWORD_LENGTH;
	device_extension->key.Buffer = ExAllocatePoolWithTag(NonPagedPool, PASSWORD_LENGTH, IRON_STORAGE_POOL_TAG);

	if (device_extension->file_name.Buffer == NULL || device_extension->key.Buffer == NULL)
	{
		return STATUS_INSUFFICIENT_RESOURCES;
	}
    ...
    if (!NT_SUCCESS(status))
    {
		...
		ExFreePool(device_extension->key.Buffer);
        Irp->IoStatus.Status = status;
        Irp->IoStatus.Information = 0;
        return status;
    }
    ...
    if (status == STATUS_OBJECT_NAME_NOT_FOUND || status == STATUS_NO_SUCH_FILE)
    {
        ...
        {
            DbgPrint("IronStorage: File %.*S not found.\n", ufile_name.Length / 2, ufile_name.Buffer);
            ExFreePool(device_extension->file_name.Buffer);
			ExFreePool(device_extension->key.Buffer);
        ...
        else
        {
            ...
            if (!NT_SUCCESS(status))
            {
                DbgPrint("IronStorage: File %.*S could not be created.\n", ufile_name.Length / 2, ufile_name.Buffer);
                ExFreePool(device_extension->file_name.Buffer);
				ExFreePool(device_extension->key.Buffer);
                RtlFreeUnicodeString(&ufile_name);
                return status;
            ...
            {
                KdPrint(("IronStorage: File %.*S created.\n", ufile_name.Length / 2, ufile_name.Buffer));
                ...
                if (NT_SUCCESS(status))
                {
                    KdPrint(("IronStorage: File attributes set to sparse.\n"));
                }
                ...
                if (!NT_SUCCESS(status))
                {
                    DbgPrint("IronStorage: eof could not be set.\n");
                    ExFreePool(device_extension->file_name.Buffer);
					ExFreePool(device_extension->key.Buffer);
                    RtlFreeUnicodeString(&ufile_name);
                ...
                KdPrint(("IronStorage: eof set to %I64u.\n", file_eof.EndOfFile.QuadPart));
            }
        }
    }
    else if (!NT_SUCCESS(status))
    {
        DbgPrint("IronStorage: File %.*S could not be opened.\n", ufile_name.Length / 2, ufile_name.Buffer);
        ExFreePool(device_extension->file_name.Buffer);
		ExFreePool(device_extension->key.Buffer);
        RtlFreeUnicodeString(&ufile_name);
        return status;
    ...
    {
        DbgPrint("IronStorage: Warning: File is compressed or encrypted. File attributes: %#x.\n", file_basic.FileAttributes);
/*
        ExFreePool(device_extension->file_name.Buffer);
		ExFreePool(device_extension->key.Buffer);
        ZwClose(device_extension->file_handle);
