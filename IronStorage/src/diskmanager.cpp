                       ...
                       unsigned int deviceId) const
{
    ...
    POPEN_FILE_INFORMATION openFileInformation = reinterpret_cast<POPEN_FILE_INFORMATION>(new char[SIZE_STRUCT_INFO]);
    ...
    // here we must add unicode supprort because application does not work with wide characters
    // also add WCHAR in library header + check all memory allocations  :to fix 6/11/2019
